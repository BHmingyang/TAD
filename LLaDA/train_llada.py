...
class TrajectoryDataset(Dataset):
    def __init__(self, data_path: str, tokenizer: AutoTokenizer, delta: int = 4, mask_token_id: int = 126336, sample_ratio: float = 1.0, shuffle: bool = True, seed: int = 42):
        ...
        # Shuffle index_map globally to randomize training data order
        if self.shuffle:
    ...
    def __getitem__(self, idx: int) -> Dict[str, torch.Tensor]:
        ...
        if prompt.startswith("<|startoftext|>"):
            system_prompt = f"<|startoftext|><|start_header_id|>system<|end_header_id|>\n\nReference Answer: {answer_gt}<|eot_id|>"
            prompt_without_bos = prompt[len("<|startoftext|>"):]
            teacher_prompt_text = system_prompt + prompt_without_bos
        else:
            system_prompt = f"<|start_header_id|>system<|end_header_id|>\n\nReference Answer: {answer_gt}<|eot_id|>"
            teacher_prompt_text = system_prompt + prompt
...
def build_collator(tokenizer: AutoTokenizer):
    ...
    def collate_fn(batch: List[Dict[str, torch.Tensor]]) -> Dict[str, torch.Tensor]:
        ...
        student_att_mask = (student_batch != pad_id).long()
        teacher_att_mask = (teacher_batch != pad_id).long()

        return {
            "student_input_ids": student_batch,
            "student_attention_mask": student_att_mask,
            "teacher_input_ids": teacher_batch,
            "teacher_attention_mask": teacher_att_mask,
            "labels": labels_batch,
...
class DLMTrainer(Trainer):
    def __init__(self, teacher_model, lambda_ce=1.0, lambda_kl=1.0, tau=1.0, mask_token_id=126336, **kwargs):
    ...
    def compute_loss(self, model, inputs, return_outputs=False, **kwargs):
        # Ensure Teacher model is on the correct GPU (DeepSpeed ZeRO-2 compatible)
...
def main():
    config = load_config("configs/config_llada.yaml")
    ...
    train_dataset = TrajectoryDataset(
        data_path="/home/Traj-Distill/data/LLaDA_data.jsonl",
        tokenizer=tokenizer,
        delta=8, 
